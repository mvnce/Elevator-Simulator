...
void CController::Service()
{
	...
	if (CElevatorController::IsFireMode())
	{
		for (int floor = 1; floor <= NumFloors; floor++)
		{
			mFloors[floor - 1].SetUp(false);
			mFloors[floor - 1].SetDown(false);
			mFloors[floor - 1].SetPanel(false);
		}	
	}
	...
	case DoorOpen:
		if (CElevatorController::IsFireMode() && mFloor == 1)
		{
			SetState(FireModeOpen);
		}

		else if (CElevatorController::IsFireMode())
		{
			SetState(FireModeClosing);
		}
	...
	case Idle:
	{
		...
		if (CElevatorController::IsFireMode())
		{
			SetState(FireModeClose);
		}

		else if (floor == mFloor)
		{
	...
	case FireModeOpen:
	{
		if (!CElevatorController::IsFireMode() && mStateTime >= DoorOpenTime)
		{
				SetState(DoorClosing);
		}
